...
cd "$PROJECT_ROOT"
python -c "from core.apis.services.database import init_db; init_db()" 2>/dev/null || echo -e "${YELLOW}Database already initialized${NC}"
...
uvicorn core.apis.services.main:app --host 0.0.0.0 --port 8000
